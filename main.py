...
x_ids, feat_dict_xtrain, feat_dict_xtest, y_train, y_test = data_reader(path)

# put data into class, add/remove features as desired
MyData_train = BloodDonations(feat_dict_xtrain,y_train)
MyData_test = BloodDonations(feat_dict_xtest,y_test)
...
data_writer(MyData_predict.x,y_submit_predicted,'data/submissions/'+nameOfExperiment+'.csv')

# -> probs predictions
y_submit_predicted_probs = trained_model.predict_proba(MyData_predict.x)[:,1]
data_writer(MyData_predict.x,y_submit_predicted_probs,'data/submissions/'+nameOfExperiment+'_probs.csv')
